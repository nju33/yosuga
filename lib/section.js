...
${this.__getAltFilename(this.filename.stylus)} was not found.
  ...
  async compile(): Promise<{[target: string]: string}> {
    if (typeof this.items.postcss !== 'undefined') {
      ...
      const altCode = await fs.readFile(altFilename, 'utf-8');
      this.altItems.postcss = altCode;
      const content = await this.__compilePostcss(altCode, this.postcssPlugins);
      this.altCompiledItems.postcss = content;
    }
    if (typeof this.items.scss !== 'undefined') {
      const altFilename = this.__getAltFilename(this.filename.scss);
      await fs.access(altFilename, fs.constants.F_OK);
      const altCode = await fs.readFile(altFilename, 'utf-8');
      this.altItems.scss = altCode;

      const dirname = this.dirname.scss;
      const content = await this.__compileScss(altCode, dirname);
      this.altCompiledItems.scss = content;
    }
    if (typeof this.items.less !== 'undefined') {
      const altFilename = this.__getAltFilename(this.filename.less);
      await fs.access(altFilename, fs.constants.F_OK);
      const altCode = await fs.readFile(altFilename, 'utf-8');
      this.altItems.less = altCode;

      const dirname = this.dirname.less;
      const content = await this.__compileLess(altCode, dirname);
      this.altCompiledItems.less = content;
    }
    if (typeof this.items.stylus !== 'undefined') {
      ...
      const content = await this.__compileStylus(altCode, dirname);
      this.altCompiledItems.stylus = content;
    }

    const result = Object.keys(this.altCompiledItems).reduce(
      ($result, target) => {
        const targetCode = this.altCompiledItems[target];
        if (targetCode === '') {
          return $result;
        }
...
${line}
  ...
  __diffTable(targets: string[], contents: string[]): string {
    ...
    while (targets.length > n) {
      rows.push([chalk.blue('∎ ') + targets[n], contents[n]]);
      n++;
    }
