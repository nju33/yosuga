...
export default class Section {
  ...
  get css(): string {
    switch (this.main) {
      ...
      case 'postcss': {
        if (typeof this.altItems.postcss === 'undefined') {
          throw new TypeError(`
${this.__getAltFilename(this.filename.postcss)} was not found.
          `.trim());
        }
        return this.altItems.postcss;
      }
      case 'scss': {
        if (typeof this.altItems.scss === 'undefined') {
          throw new TypeError(`
${this.__getAltFilename(this.filename.scss)} was not found.
          `.trim());
        }
        return this.altItems.scss;
      }
      case 'less': {
        if (typeof this.altItems.less === 'undefined') {
          throw new TypeError(`
${this.__getAltFilename(this.filename.less)} was not found.
          `.trim());
        }
        return this.altItems.less;
      }
      case 'stylus': {
        if (typeof this.altItems.stylus === 'undefined') {
          throw new TypeError(`
${this.__getAltFilename(this.filename.stylus)} was not found.
          `.trim());
        }
        return this.altItems.stylus;
      }
    }
  }
  ...
  set(target: string, contents: string, filename: string) {
    ...
    this.dirname[target] = path.dirname(filename);
  ...
  async compile(): Promise<{[target: string]: string}> {
    if (typeof this.items.postcss !== 'undefined') {
      ...
      const $altItems = await fs.readFile(altFilename, 'utf-8');
      const content = await this.__compilePostcss(
        $altItems,
        this.postcssPlugins
      );
      this.altItems.postcss = content;
    }
    if (typeof this.items.scss !== 'undefined') {
      ...
      const content = await this.__compileScss($altItems, dirname);
      this.altItems.scss = content;
    }
    if (typeof this.items.less !== 'undefined') {
      ...
      const content = await this.__compileLess($altItems, dirname);
      this.altItems.less = content;
    }
    if (typeof this.items.stylus !== 'undefined') {
      ...
      const content = await this.__compileStylus($altItems, dirname);
      this.altItems.stylus = content;
    }

    const result = Object.keys(this.altItems).reduce(($result, target) => {
      if (this.altItems[target] === '') {
        return $result;
      }
      $result[target] = this.altItems[target];
      return $result;
    }, {});
    return result;
  }
  ...
  async isValid(targetData: {[target: string]: string}): Promise<boolean> {
    ...
    const rawContents = await (async () => {
      ...
      if (this.main === 'css') {
        const cssAltItems = this.standardize(this.items.css);
        return [cssAltItems, ...$rawContents];
      }
      return $rawContents;
    })();
