...
export default class View {
  ...
  constructor(opts: {[string]: any}) {
    ...
    this.config.css = ['~static/base.css'];
  ...
  async build(): Promise<any> {
    // (this.config.build: any).extend = config => {
    //   config.module.rules.map(rule => {
    //     if (rule.loader === 'babel-loader') {
    //       config.module.rules[0].options.loaders.js.exclude = /node_modules(?!\/vue-octicon\/icons)/;
    //       rule.exclude = /node_modules(?!\/vue-octicon\/icons)/;
    //     }
    //     return rule;
    //   });
    //   return config;
